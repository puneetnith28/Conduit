...
/**
 * Build a Codex agent's Conduit environment: the `developerInstructions`
...
function buildAgentEnv(agent: Agent): { developerInstructions: string; config: Record<string, unknown> } {
  ...
  const developerInstructions = [
    '# Conduit — your environment',
    '',
    `You are **${agent.name}**, an agent on the **${projectName}** team inside`,
    'Conduit, a dashboard that manages a team of coding agents.',
    '',
    ...
    '(the Conduit orchestrator) — if you need something from a teammate, say so',
...
const HISTORY_DIR = path.join(os.homedir(), '.conduit', 'codex-history');
